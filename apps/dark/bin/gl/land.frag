...
in vec4 vLandscape0123;
in vec4 vLandscape4567;
...
void main()
{
	vec2 offset = (vPos / 4096.0) - landOffset;
	vec3 color2 = vec3(2.0) * offset.x * offset.y;

	vec4 diffuseColor = vec4(color, 1.0);
	//vec4 diffuseColor = vec4(color2, opacity);

	//diffuseColor *= vLandscape0123[3];
	...
	float fogNear = 2000.0 / ONE_SKYRIM_UNIT_IN_CM;
	float fogFar = 50000.0 / ONE_SKYRIM_UNIT_IN_CM;
	float fogDensity = 0.0001; // 0.0022 for dark dungeon 0.0011 for light
	...
	material.specularColor = specular + vec3(0.5, 0.5, 0.5);
